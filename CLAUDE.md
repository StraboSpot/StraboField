...
#### How the Changelog Works (`.github/workflows/changelog.yml`)

The "Publish Release" workflow triggers on the `v*` tag push (not on the master push) and generates the release notes:

- **Scope:** it diffs against the **immediately preceding version tag** in `sort -V` order, so a patch shows only the commits since the previous patch (not everything since the last minor). This is why the changelog fix in Step 3 must already be on master **before** you tag — the Action runs the workflow file from the tagged commit.
- **Output:** notes are published to the **GitHub Release page only**. The workflow does **not** commit a `CHANGELOG.md` to the repo — `master` is a protected branch, so the `github-actions[bot]` push would be rejected. The Release page is the source of truth.
- **Filtering:** `chore(release|workflows|pods|docs)`, `chore: bump version`, `chore: update changelog`, and several noisy `fix:` prefixes are stripped from the notes. Write meaningful commit subjects (`feat(...)`, `fix(...)`) — they become the changelog verbatim.
- **First release edge case:** if no previous tag exists, it logs the full history up to the tag.
...
1. `npm run bump-[patch|minor|major]` - Updates all version files (package.json + iOS/Android via Fastlane)
2. `npm run commit-and-push` - Commits changes and pushes to master (runs `scripts/commit-and-push.js`, which stamps the bumped version into the commit message)
3. `npm run deploy-beta` - Deploys via Fastlane (requires Fastlane setup)

The changelog is **not** created by these scripts — it is generated by the `Publish Release` GitHub Action when you push the `v{version}` tag. See [How the Changelog Works](#how-the-changelog-works-githubworkflowschangelogyml) above.
