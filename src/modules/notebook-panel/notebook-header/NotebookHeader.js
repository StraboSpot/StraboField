...
const NotebookHeader = ({
  ...
  // Name the dataset that locks this record AND why the record being read is affected by it. Without the reason,
  // someone looking at a Spot in their own open dataset is sent to a dataset that Spot is not even in. Read Only now
  ...
  const getReadOnlyReasonText = () => {
    ...
    if (cause === 'map') {
      ...
      return `The ${mapText} this ${spotLabel} is on belongs to a Read Only Spot. ${datasetText}`;
    }
    return `This ${spotLabel} is in a Read Only dataset. ${datasetText}`;
  };
