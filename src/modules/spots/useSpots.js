...
const useSpots = () => {
  ...
  // Why a Spot is read only, so the notebook can say more than which dataset it is in. cause is:
  //   project      - the whole project is, which locks every dataset in it
  ...
  const getReadOnlyReason = (spot) => {
    ...
    // A read only project locks every dataset, so there is no walk worth doing and no one dataset to name
    if (isReadOnlyProject) return {cause: 'project', datasetNames: []};
  ...
  const isSpotReadOnly = (spot) => {
    // Runs once per row of a Spot list, so skip the walk when the answer is settled - nothing is read only,
    // or everything is. isEmpty stays first: the geo map passes no holding Spot, and has none to lock it
